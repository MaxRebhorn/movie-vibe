...
class EmbeddingTests(TestCase):
    ...
    def test_embed_data_with_mixed_input(self):
        # Import only after skip check
        from movies.services.embed_service import embed_data

        with patch("movies.services.embed_service.model.encode") as mock_encode:
    ...
    def test_embed_model(self):
        # Import only after skip check
        from movies.services.embed_service import embed_model, embed_data

        with patch("movies.services.embed_service.embed_data") as mock_embed_data:
